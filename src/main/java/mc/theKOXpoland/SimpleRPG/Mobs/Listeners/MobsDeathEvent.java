<<<<<<< HEAD:src/main/java/mc/theKOXpoland/SimpleRPG/Mobs/Listeners/MobsDeathEvent.java
    ...
    public void onCreatureDeath(EntityDeathEvent event) {
        ...
        if (dedEntity.getPersistentDataContainer().has(plugin.Key_NBT_Name)) {
            for (String key : MobManager.customMobMap.keySet()) {
                String nbtName = dedEntity.getPersistentDataContainer().get(plugin.Key_NBT_Name, PersistentDataType.STRING);
                if (nbtName != null && nbtName.contains(key)) {
                    CustomMob killedMob = MobManager.customMobMap.get(key);
                    ...
                    for (String wepString :  killedMob.getMobDrop()) {
                        ...
                        for (String dropedItem : CustomItemsManager.getAllCustomItemsMap().keySet()) {
                            if (splitedWeapons[0].equals(dropedItem)) {
                                if (!rangeItem.containsValue(CustomItemsManager.getAllCustomItemsMap().get(dropedItem))) {
                                    ...
                                    range = ValueRange.of((long) firstNumber, (long) totalChance);
                                    rangeItem.put(range, CustomItemsManager.getAllCustomItemsMap().get(dropedItem));
...
>>>>>>> main:src/main/java/mc/theKOXpoland/SimpleRPG/Listeners/MobsDeathEvent.java
